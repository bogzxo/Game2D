...
public partial class Shader : IDisposable
{
    ...
    public static Shader CreateShader(params (ShaderType Type, string Path)[] shaders)
    {
        return new Shader(shaders);
    }

    private Shader(params (ShaderType Type, string Path)[] shaders)
    {
        Files = shaders;
    ...
    private int CreateProgram(params (ShaderType Type, string source)[] shaderPaths)
    {
        ...
        {
            Shaders[i] = CompileShader(shaderPaths[i].Type, File.ReadAllText(shaderPaths[i].source));
        }
